...
function setup_sim_param_model(args::Vector{String} = Array{String}(undef, 0)) # Allow this to take a list of parameter (e.g., from command line)
    ...
    add_param_active(sim_param,"f_stars_with_planets_attempted", 0.5)
    ...
    add_param_active(sim_param,"log_rate_clusters", log(0.8))
    add_param_fixed(sim_param,"max_clusters_in_sys", 10)
    add_param_active(sim_param,"log_rate_planets_per_cluster", log(3.7))
    ...
    add_param_active(sim_param,"power_law_r1", -1.0)
    add_param_active(sim_param,"power_law_r2", -4.4)
    ...
    add_param_active(sim_param,"sigma_incl_near_mmr", 1.4)
    ...
    add_param_active(sim_param,"sigma_hk", 0.02)
